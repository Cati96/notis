...
public class NotaryService {
    ...
    public List<Notary> getAllByAddress(String country, String county, String city){
        return SparkQLQuery.getAllNotaries(country.toUpperCase(), county.toLowerCase(), city.toUpperCase());
    }
    ...
    private Notary getDefaulNotary(EntityDTO entityDTO, Integer id) {
        ...
        Notary notary = new Notary(id, entityDTO.getName(), "" + entityDTO.getAuthorizationNumber(), entityDTO.getPhoneNumber(), address, LiteralConvertor.convertFromStringToTimetable(schedule), LiteralConvertor.convertFromStringToServices(services));
        return notary;
    }
