...
public class EntityCreator {
    public static EntityObject createEntityToBeStored(EntityDTO entityDTO, boolean isTranslator) {
        ...
        EntityObject entityObject = EntityObject.builder().id(id).entityType(entityID).name(entityDTO.getName())
                .authorizationNumber(entityDTO.getAuthorizationNumber()).phoneNumber(entityDTO.getPhoneNumber())
                .address(address)
                .timetable(
                        "['10:00 - 14:00 | 15:00 - 18:00', '8:30 - 13:00 | 14:00 - 19:00', '9:30 - 12:00 | 13:00 - 19:00', '9:00 - 13:00 | 14:00 - 19:30', '9:00 - 13:00 | 14:00 - 19:30', '9:30 - 12:00 | 13:00 - 19:00', None]")
                .services(offeredServices).languages(languages).build();
        return entityObject;
    }
    ...
    public static EntityObject createEntityToBeStored(Notary notary, boolean isTranslator) {
        ...
        EntityObject entityObject = EntityObject.builder().id(id).entityType(entityID).name(notary.getName())
                .authorizationNumber(Integer.parseInt(notary.getAuthorizationNumber()))
                .phoneNumber(notary.getPhoneNumber()).address(notary.getAddress())
                .timetable(convertTimeTableToString(notary.getTimetable()))
                .services(convertServicesToString(notary.getServices())).languages("-").build();
        return entityObject;
    }
    ...
    public static EntityObject createEntityToBeStored(Translator translator, boolean isTranslator) {
        ...
        Address address = new Address("-", "-", "-", "-", "-", "-", "-", "-");
        EntityObject entityObject = EntityObject.builder().id(id).entityType(entityID).name(translator.getName())
                .authorizationNumber(Integer.parseInt(translator.getAuthorizationNumber()))
                .phoneNumber(translator.getPhoneNumber()).address(translator.getAddress())
                .timetable(convertTimeTableToString(translator.getTimetable()))
                .services(convertServicesToString(translator.getServices()))
                .languages(convertLanguagesToString(translator.getLanguages())).build();
        return entityObject;
    }

    private static String convertTimeTableToString(Timetable timetable) {
        StringBuilder sb = new StringBuilder();
        sb.append("[").append(timetable.getMonday()).append(", ").append(timetable.getThursday()).append(", ")
                .append(timetable.getWednesday()).append(", ").append(timetable.getThursday()).append(", ")
                .append(timetable.getFriday()).append(", ").append(timetable.getSaturday()).append(", ")
                .append(timetable.getSunday()).append("]");
        return sb.toString();
    }

    private static String convertServicesToString(List<Service> services) {
        String servicesString = services.stream()
                .map(service -> "['" + service.getType() + "', '" + service.getDescription() + "']")
...
//        "['FRANCEZĂ', 'RUSĂ']"
        String languagesString = languages.stream().map(language -> "'" + language.toUpperCase().replace(",", "") + "'")
                .collect(Collectors.joining(", "));
